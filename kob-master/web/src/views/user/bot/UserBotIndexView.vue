<template>
    <div class="container">
        <div class="row">
            ...
            <div class="col-9">
                <div class="card" style="margin-top: 20px;">
                    <div class="card-header">           <!--想修改文本的样式，就要奖文本用span括起来, float-end实现向右对齐-->
                        <span style="font-size: 130%">我的Bot</span>            <!--实现点某个页面打开页面，就要用到下面这两个东西,用id来打开-->
                        ...
                        <div class="modal fade" id="add-bot-btn" tabindex="-1">
                            <div class="modal-dialog modal-xl">
                                ...
                                <div class="modal-body">
                                    <div class="mb-3">
                                        <label for="add-bot-title" class="form-label">名称</label> <!--placeholder 是在输入框里的文字-->
                            ...
                            <tbody>
                                <tr v-for="bot in bots" :key="bot.id">
                                    ...
                                    <td> <!--修改的模态框-->
                                        ...
                                        <button type="button" class="btn btn-danger" @click="remove_bot(bot)">删除</button> <!--绑定remove_bot函数，传参数，只要加个括号就行-->

                                        <div class="modal fade" :id="'update-bot-modal-' + bot.id" tabindex="-1"> <!--因为每个bot的模态框值不一样,所以加上bot.id，保证每个模态框都不一样-->
                                            <div class="modal-dialog modal-xl">
                                                <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title">创建Bot</h5>
...
export default {
    ...
    setup() {
        ...
        const add_bot = () => {
            ...
            $.ajax({
                url: "https://app5375.acapp.acwing.com.cn/api/user/bot/add/",
                type: "post",
                data: {
                    title: botadd.title,
                    description: botadd.description, //reactive对象取值不需要用value
                ...
                success(resp) {
                    if (resp.error_message === "success") { //创建成功后，将内容清空
                        botadd.title = "";
                        botadd.description = "";
                        botadd.content = "";
                        Modal.getInstance("#add-bot-btn").hide(); //调用将模态框关闭的api
                        refresh_bots(); //每次删除或添加成功后，刷新列表
                    } else {
        ...
        const remove_bot = (bot) => {
            $.ajax({
                url: "https://app5375.acapp.acwing.com.cn/api/user/bot/remove/",
                type: "post",
                data: {
                    bot_id: bot.id,
                },
